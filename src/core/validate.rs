...
fn validate_project_specs_docs(
    ...
    if let Some(manifest) = manifest {
        ...
        let template_is_current =
            manifest.template_version == crate::core::scaffold::PROJECT_SPEC_TEMPLATE_VERSION;
        if template_is_current {
            pass("Project specs template version is current", ctx);
        } else if refresh_specs {
            info(
                "STALE_SPECS_TEMPLATE: Project specs were generated by an older template version. Regenerating project specs...",
            );
            let _ = crate::core::scaffold::refresh_project_specs_from_config(repo_root)?;
            specs_refreshed = true;
            pass(
                "Project specs regenerated due to template-version drift",
                ctx,
            );
        } else {
            fail(
                &format!(
                    "STALE_SPECS_TEMPLATE: Project specs were generated by template version '{}' but current template version is '{}'. Review and run `decapod rpc --op specs.refresh` or `decapod validate --refresh-specs`.",
        ...
        if current_repo_fp == manifest.repo_signal_fingerprint {
            ...
            pass(
                "Project specs manifest repo-signal fingerprint refreshed with regenerated specs",
                ctx,
            );
        } else if refresh_specs {
            info(
                "STALE_SPECS_FINGERPRINT: Significant repo surfaces changed since last specs refresh. Regenerating project specs...",
            );
            let _ = crate::core::scaffold::refresh_project_specs_from_config(repo_root)?;
            pass(
                "Project specs regenerated and manifest refreshed due to stale fingerprint",
                ctx,
            );
        } else {
            fail(
