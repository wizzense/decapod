...
| claim.proof.acceptance_evidence_input | Acceptance scenarios, generated tests, binding validation, runner output, and mutation reports are proof inputs subordinate to Decapod intent, boundaries, and proof policy. | `plugins/VERIFY.md` | partially_enforced | `decapod qa verify` file artifact drift checks + supported proof gates | First-class acceptance proof replay is planned; current support records and verifies referenced artifacts. |
| claim.recursion.bounded_authorized_pass | Recursive improvement loops are valid only as constitution-authorized, parent-linked, scope-bounded, stop-conditioned, proof-backed pass artifacts. | `docs/ARCHITECTURE_OVERVIEW.md` | enforced | `decapod validate` (recursive improvement pass gate) | Prevents free-form self-improvement loops from mutating intent, weakening governance, or polishing indefinitely. |
